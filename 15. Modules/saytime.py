...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 19 16:24:10 2019
@author: Mehedi Hasan Akash
Github: https://github.com/MehediMehi
"""
# if __name__ == "__main__": main() prevents the main code from being run when it is imported as a module
# we can import other classes and modules (functions) except main() as we don't need that anyway (main() runs this file where importing this file lets other file use this file properties)

import sys
import time

__version__ = "1.1.0" # giving a version number for this module

class numwords(): # formats a number into words
    """
        return a number as words,
        e.g., 42 becomes "forty-two"
    """ # when you type help(numwords) in the terminal, this message will be shown
    _words = {
        'ones': (
...
class saytime_t(saytime):   # (special version of saytime) wrapper for saytime to use time object
    """
...
def main(): # to run from the command line
    if len(sys.argv) > 1:
        if sys.argv[1] == 'test':
            test()
        else:
...
def test(): # test function for testing
    print("\nnumbers test:")
    list = (
